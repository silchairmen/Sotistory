...
export default function SignUp() {
  ...
  const handleSignUp = async (event) => {
    ...
    try {
      ...
      // 회원가입 요청 보내기
      const response = await axios.post('http://www.soti.or.kr/api/member/join', formData);
  ...
  const CheckNickName = async () => {
    try {
      ...
      const resp = await axios.post('http://www.soti.or.kr/api/member/help/check_nickname', Data1);
  ...
  const   CheckEmail = async () => {
    try {
      ...
      const resp = await axios.post('http://www.soti.or.kr/api/member/help/check_email', Data2);
      if (resp.data.status === 200) {
